...
const MatxVerticalNavExpansionPanel = ({ item, children, mode }) => {
	...
	return (
		<div>
			<TouchRipple
				className={clsx({
					"flex justify-between h-44 border-radius-4 mb-2 w-full pr-4 has-submenu compactNavItem whitespace-pre overflow-hidden font-medium": true,
			...
			>
				<div className="flex items-center">
					{icon && <Icon className="align-middle text-18 w-36 px-4">{icon}</Icon>}
					{iconText && <div className="w-4 h-4 rounded ml-5 mr-4"></div>}
					<span className={clsx("align-middle sidenavHoverShow", classes.itemText)}>{name}</span>
				</div>
				{badge && (
					<div className={clsx(`rounded bg-${item.badge.color} px-1 py-1px`, "sidenavHoverShow", classes.itemIcon)}>{badge.value}</div>
				)}
				<div
					className={clsx({
			...
			<div
				ref={elementRef}
				className={clsx({
					[classes["expansion-panel"]]: true,
					submenu: true,
					[classes.subMenuIcon]: mode !== "compact",
				})}
