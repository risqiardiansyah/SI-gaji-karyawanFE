...
class EditSuratPeringatan extends Component {
  constructor() {
    super();
    this.state = {
      nama_signer: "",
  ...
  componentWillReceiveProps(nextProps) {
    const dataDetailSP = nextProps.detail;
    this.setState({
      nama_karyawan: dataDetailSP?.karyawan_code,
      nama_signer: dataDetailSP?.signer_code,
      pelanggaran: dataDetailSP?.pelanggaran,
      sp_bulan: dataDetailSP?.sp_bulan,
      tgl_surat: dataDetailSP?.tgl_surat,
      sp_ke: dataDetailSP?.sp_ke,
      kop_code: dataDetailSP?.kop_code,
    });
  }
  ...
  submitData(params) {
    const { editSP, history } = this.props;
    editSP(params)
      .then((res) => {
        if (res.code === 0) {
          Swal.fire({
  ...
  render() {
    ...
    return (
      <div className="m-sm-30">
        <SimpleCard loading={false} title="Edit Surat Peringatan">
              ...
              {sp_ke == 3 || sp_ke == 1 ? (
                <Grid item sm={6} xs={12} className="mb-3">
                  <Select
                    ...
                    value={dataBulan.filter((item) => item.value === sp_bulan)}
                    name="sp_bulan"
                    options={dataBulan}
...
const mapStateToProps = (state) => {
  return {
    ...
    detail: state.peringatan.detail,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    editSP: (params) => dispatch(editSP(params)),
    checkSP: (karyawan_code) => dispatch(checkSP(karyawan_code)),
    getDetailSP: (code) => dispatch(getDetailSP(code)),
