...
import {
  ...
  delSP,
  downloadSP,
} from "app/redux/actions/PeringatanAction";

class SuratPeringatan extends Component {
  constructor() {
    super();
    this.state = {
      loading: true,
  ...
  handleDelete = (id) => {
    const { delSP, getAllSP } = this.props;
    Swal.fire({
      title: "Hapus",
  ...
  handleEdit = (param) => {
    ...
    history.push(`/surat_peringatan/edit/${param}`);
  };
  ...
  handlePrint = (item, send_email) => {
    ...
    const { downloadSP } = this.props;

    downloadSP(item.unique_code, send_email).then((res) => {
  ...
  renderTable = () => {
    ...
    const { dataSP } = this.props;

    return dataSP?.length > 0 ? (
      dataSP
        ...
        .map((item, index) => {
          return (
            <TableRow hover key={index}>
              ...
              <TableCell colSpan={2}>{"SP-" + item.sp_ke}</TableCell>
              <TableCell colSpan={2}>
                {formatTanggal(item.created_at)}
              </TableCell>
              <TableCell>{item.karyawan_position}</TableCell>
  ...
  handleAdd = () => {
    const { history } = this.props;
    history.push("/surat_peringatan/tambah");
  };
    ...
    return (
      <div className="m-sm-30">
        <SimpleCard
          ...
          subtitle="Data Surat Peringatan(SP)"
          currency=""
          saldo=""
        >
          <Fragment>
            ...
            <div className="w-full overflow-auto bg-white">
              <Table className="buku-kas-table">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell colSpan={2}>Tipe</TableCell>
              ...
              <TablePagination
                ...
                component="div"
                count={dataSP?.length ? dataSP?.length : 0}
...
const mapStateToProps = (state) => {
  return {
    dataSP: state.peringatan.dataSP,
  };
};
const mapDispatchToProps = (dispatch) => {
  return {
    getAllSP: () => dispatch(getAllSP()),
    delSP: (params) => dispatch(delSP(params)),
    downloadSP: (sk_code, send_email) =>
      dispatch(downloadSP(sk_code, send_email)),
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(SuratPeringatan);
