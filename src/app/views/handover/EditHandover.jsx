...
import { editHO, getDetailHO } from "app/redux/actions/HandoverActions";
...
class EditHandover extends Component {
  constructor() {
    super();
    this.state = {
  ...
  componentDidMount() {
    const { getSigner, getKaryawan, getDetailHO, match } = this.props;
    getSigner();
    getKaryawan(3);
    getDetailHO(match.params.ho_code);
  }

  componentWillReceiveProps(nextProps) {
    const { detailHO } = nextProps;

    this.setState({
      ho_code: detailHO.ho_code,
      tgl_masuk: detailHO.tgl_masuk,
      tgl_keluar: detailHO.tgl_resign,
      alasantext: detailHO.alasan_resign,
      ho_list: detailHO.list || [],
      nama_signer: detailHO.signer_code,
      nama_karyawan: detailHO.karyawan_code,
      changed: true,
      alasan: detailHO.status,
    });
  }
  ...
  submitData(params) {
    const { editHO, history } = this.props;
    editHO(params)
      .then((res) => {
        if (res.code === 0) {
          Swal.fire({
        ...
        <SimpleCard loading={false} title="Tambah data Handover">
          <ValidatorForm ref="form" onSubmit={this.submitForm}>
            <Grid container spacing={2}>
              ...
              <Grid item sm={6} xs={12}>
                <Select
                  value={dataAlasan.filter((item) => item.value == alasan)}
                  name="alasan_resign"
                  options={dataAlasan}
...
const mapStateToProps = (state) => {
  return {
    ...
    detailHO: state.handover.detailHO,
  };
};
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getDetailHO: (ho_code) => dispatch(getDetailHO(ho_code)),
    editHO: (params) => dispatch(editHO(params)),
  };
};
