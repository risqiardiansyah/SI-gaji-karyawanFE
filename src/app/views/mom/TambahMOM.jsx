...
import { getMOMWith, createMOM } from "app/redux/actions/MOMActions";
...
class TambahMOM extends Component {
  constructor() {
    super();
    this.state = {
  ...
  componentDidMount() {
    const { getMOMWith, getSigner } = this.props;
    getMOMWith();
    getSigner();
  }
  ...
  submitData(params) {
    const { createMOM, history } = this.props;
    createMOM(params)
      .then((res) => {
        if (res.code === 0) {
          Swal.fire({
  ...
  render() {
    ...
    return (
      <div className="m-sm-30">
        <SimpleCard loading={false} title="Tambah data Minutes of Meeting(MoM)">
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    createMOM: (params) => dispatch(createMOM(params)),
    getMOMWith: (type) => dispatch(getMOMWith(type)),
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(TambahMOM);
