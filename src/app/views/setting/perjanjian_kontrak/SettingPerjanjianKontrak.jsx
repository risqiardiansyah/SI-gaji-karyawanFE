...
import {
  getPasalPerjanjianKontrak,
  savePasalPerjanjianKontrak,
  deletePasalPerjanjianKontrak,
...
class SettingPerjanjianKontrak extends Component {
  constructor() {
    super();
    this.state = {
      submit: false,
  ...
  getData() {
    const { getPasalPerjanjianKontrak } = this.props;
    getPasalPerjanjianKontrak();
  }
  ...
  handleSave = () => {
    const { dataPasal } = this.state;
    const { savePasalPerjanjianKontrak } = this.props;
    const params = {
      data: dataPasal,
    };

    savePasalPerjanjianKontrak(params)
      .then((res) => {
        if (res.code === 0) {
          Swal.fire({
            title: "Berhasil",
  ...
  handleDelete = (index, ke) => {
    const { dataPasal } = this.state;
    const { deletePasalPerjanjianKontrak } = this.props;
    if (index === undefined) {
      ...
      Swal.fire({
        ...
        if (res.isConfirmed) {
          deletePasalPerjanjianKontrak(index)
            .then((res) => {
              if (res.code === 0) {
                let newData = dataPasal.filter((item) => item.ke != ke);
                this.setState({
  ...
  render() {
    ...
    return (
      <div className="m-sm-30">
        <h3>Setting Default Pasal Perjanjian Kerja</h3>
        <div className="flex">
          <Grid
            container
            spacing={2}
...
const mapDispatchToProps = (dispatch) => {
  return {
    getPasalPerjanjianKontrak: () => dispatch(getPasalPerjanjianKontrak()),
    savePasalPerjanjianKontrak: (params) =>
      dispatch(savePasalPerjanjianKontrak(params)),
    deletePasalPerjanjianKontrak: (idx) =>
      dispatch(deletePasalPerjanjianKontrak(idx)),
  };
};
