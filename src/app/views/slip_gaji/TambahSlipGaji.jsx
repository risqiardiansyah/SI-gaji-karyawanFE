...
class TambahSlipGaji extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { kchanged } = this.state;
    // console.log("detail", nextProps.detailKaryawan);
      ...
      const newPenambahan = [
        {
          index: 0,
          nama: "Gaji Pokok",
          biaya: detailKaryawan.gaji_pokok,
          disabled: true,
        },
        {
          index: 1,
          nama: "Tunjangan",
          biaya: detailKaryawan.tunjangan,
          disabled: true,
        },
      ];
      this.setState(
        ...
        () => {
          this.calculatePPH(detailKaryawan);
        }
      );
    }
  }
  ...
  sendSubmit() {
    ...
    let params = {
      ...
      total_potongan: hasil_potongan + pph,
      kop_code: kop_code,
    };
  ...
  submitData(params) {
    const { addSlip, history } = this.props;
    addSlip(params)
      .then((res) => {
        if (res.code === 0) {
          Swal.fire({
  ...
  render() {
    ...
    // console.log("detail_karyawan", detailKaryawan);
    return (
      <div className="m-sm-30">
        <SimpleCard loading={false} title="Tambah Slip Gaji">
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    addSlip: (params) => dispatch(addSlip(params)),
    getAllHeader: (type) => dispatch(getAllHeader(type)),
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(TambahSlipGaji);
