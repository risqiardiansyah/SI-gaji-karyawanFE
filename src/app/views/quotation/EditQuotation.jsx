...
class EditQuotation extends Component {
  ...
  componentDidMount() {
    ...
    getDetailQuotation(match.params.id);
  ...
  sendSubmit() {
    ...
    const params = {
      ...
      quotation_code: match.params.id,
      currency: currency,
      bahasa: bahasa,
      kop_code: kop_code,
    };
  ...
  submitData(params) {
    const { updateQuotation, history } = this.props;
  ...
  componentWillReceiveProps(nextProps) {
    const dataDetailQuotation = nextProps.detailQuotation;

    this.setState({
      nama_mitra: dataDetailQuotation?.mitra_code,
      nama_signer: dataDetailQuotation?.signer_code,
      quotation_date: dataDetailQuotation?.quotation_date,
      about: dataDetailQuotation?.about,
      total_biaya: dataDetailQuotation?.total,
      quotation_item: dataDetailQuotation?.quotation_item || [],
      currency: dataDetailQuotation?.currency,
      bahasa: dataDetailQuotation?.bahasa,
      kop_code: dataDetailQuotation?.kop_code,
    });

    console.log("quotation_item", dataDetailQuotation?.quotation_item);
  }
  ...
  render() {
    ...
    return (
      <div className="m-sm-30">
        <SimpleCard loading={false} title="Edit Quotation">
          <ValidatorForm ref="form" onSubmit={this.submitForm}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item sm={6} xs={12}>
                <Select
                  value={dataCurrency.filter((item) => item.value === currency)}
                  name="currency"
...
const mapStateToProps = (state) => {
  return {
    ...
    detailQuotation: state.quotation.detail,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getDetailQuotation: (code) => dispatch(getDetailQuotation(code)),
    addQuotation: (params) => dispatch(addQuotation(params)),
    updateQuotation: (params) => dispatch(updateQuotation(params)),
