...
class EditSertifikat extends Component {
  ...
  componentDidMount() {
    ...
    getKaryawan(3);
    getOfficeList();
    getDetailSertifikat(match.params.id);
    this.setState({
      set: 1,
    });
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { dataSigner, dataKaryawan, dataOffice, detailSertifikat } =
      nextProps;
    const { set } = this.state;

    if (Object.keys(detailSertifikat).length != 0 && set != 0) {
      let start = JSON.stringify(detailSertifikat?.dari_bulan);
      start = start.slice(1, 11);
      let end = JSON.stringify(new Date(detailSertifikat?.sampai_bulan));
      end = end.slice(1, 11);
      ...
      this.setState({
        ...
        from_date: start,
        to_date: end,
        office_code: detailSertifikat?.office_code,
        nama_signer: detailSertifikat?.signer_code,
        nama_karyawan: detailSertifikat?.karyawan_code,
        position: detailSertifikat?.posisi,
        set: 0,
      });
    }
  }
  ...
  sendSubmit() {
    ...
    let params = {
      ...
      sertifikat_code: match.params.id,
    };
  ...
  submitData(params) {
    const { editSertifikat, history } = this.props;
    editSertifikat(params)
      .then((res) => {
        if (res.code === 0) {
          Swal.fire({
            title: "Berhasil",
  ...
  render() {
    ...
    return (
      <div className="m-sm-30">
        <SimpleCard loading={false} title="Edit Sertifikat">
          <ValidatorForm ref="form" onSubmit={this.submitForm}>
            <Grid container spacing={2}>
              <Grid item sm={6} xs={12}>
                <Select
                  defaultValue={dataOffice.filter(
                    (item) => item.value == office_code
                  )}
                  value={dataOffice.filter((item) => item.value == office_code)}
                  name="office_code"
                  options={dataOffice}
              ...
              <Grid item sm={6} xs={12}>
                <Select
                  defaultValue={dataSigner.filter(
              ...
              <Grid item sm={6} xs={12}>
                <Select
                  defaultValue={dataKaryawan.filter(
...
const mapStateToProps = (state) => {
  return {
    ...
    detailSertifikat: state.suket.detailSertifikat,
  };
};
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    editSertifikat: (params) => dispatch(editSertifikat(params)),
    getDetailSertifikat: (code) => dispatch(getDetailSertifikat(code)),
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(EditSertifikat);
