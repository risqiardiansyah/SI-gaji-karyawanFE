...
class TambahGaransi extends Component {
  ...
  componentDidMount() {
    const { getSigner, dataSigner, getPelanggan, dataPelanggan, getAllHeader } =
      this.props;
  ...
  submitData(params) {
    const { addGaransi, history } = this.props;
    addGaransi(params)
      .then((res) => {
        if (res.code === 0) {
          Swal.fire({
  ...
  render() {
    ...
    return (
      <div className="m-sm-30">
        <SimpleCard loading={false} title="Tambah Surat Garansi">
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    addGaransi: (params) => dispatch(addGaransi(params)),
    getAllHeader: (type) => dispatch(getAllHeader(type)),
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(TambahGaransi);
